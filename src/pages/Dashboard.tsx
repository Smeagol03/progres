  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      if (!error && paketList) {
        ...
        const total_pagu_anggaran = paketList.reduce((sum, p) => sum + Number(p.pagu_anggaran || 0), 0);
        const total_nilai_kontrak = paketList.reduce((sum, p) => sum + Number(p.nilai_kontrak || 0), 0);
        const total_nilai_terbayar = paketList.reduce((sum, p) => sum + Number(p.nilai_terbayar || 0), 0);

        // Capaian dihitung dari Pagu Anggaran agar lebih akurat
        const capaian_keuangan_persen = total_pagu_anggaran > 0 ? (total_nilai_terbayar / total_pagu_anggaran) * 100 : 0;
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-navy font-display mb-2">Dashboard Capaian</h1>
          <p className="text-gray-500 text-sm md:text-base">Ringkasan progres pembayaran program RTLH</p>
        </div>
        {data.total_paket === 0 && (
          <Link to="/paket" className="bg-navy hover:bg-navy-light text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm hover:shadow-md shrink-0">
      ...
      <div className="bg-white rounded-3xl p-8 mb-8 border border-gray-100 shadow-sm relative overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="text-sm text-gray-500 mt-2 text-center max-w-md">
            Total Dana Terbayar: {formatRupiah(data.total_nilai_terbayar)} <br/>
        ...
        {topUnpaid.length === 0 ? (
          ...
          <div className="divide-y divide-gray-50">
            {topUnpaid.map((paket) => {
              ...
              return (
                <div key={paket.id} className="p-4 md:p-6 hover:bg-gray-50/50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3 group">
                  <div className="min-w-0">
                    <h3 className="font-semibold text-navy mb-1 group-hover:text-gold transition-colors text-sm md:text-base truncate">{paket.nama_paket}</h3>
                    <p className="text-xs md:text-sm text-gray-500 truncate">{paket.lokasi || 'Lokasi belum diset'}</p>
                  </div>
