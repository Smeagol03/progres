...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      // Fetch all packages for aggregation
      ...
      if (!error && paketList) {
        ...
        const paket_selesai = paketList.filter(p => p.status_fisik === 'selesai').length;
        const paket_lunas = paketList.filter(p => p.nilai_kontrak > 0 && p.nilai_terbayar >= p.nilai_kontrak).length;
        
        const total_nilai_kontrak = paketList.reduce((sum, p) => sum + Number(p.nilai_kontrak), 0);
        const total_nilai_terbayar = paketList.reduce((sum, p) => sum + Number(p.nilai_terbayar), 0);

        const capaian_fisik_persen = total_paket > 0 ? (paket_selesai / total_paket) * 100 : 0;
        ...
        setData({
          ...
          capaian_fisik_persen,
          capaian_keuangan_persen
        });

        // Top unpaid packages
        const unpaidList = paketList
          .filter(p => Number(p.nilai_terbayar) < Number(p.nilai_kontrak))
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-navy font-display mb-2">Dashboard Capaian</h1>
          <p className="text-gray-500">Ringkasan progres fisik dan keuangan program RTLH</p>
        </div>
        {data.total_paket === 0 && (
          <Link to="/paket" className="bg-navy hover:bg-navy-light text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm hover:shadow-md">
      ...
      <div className="bg-white rounded-3xl p-8 mb-8 border border-gray-100 shadow-sm relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
          <div className="flex flex-col items-center">
            <ProgressRadial 
              progress={data.capaian_fisik_persen} 
              label="Capaian Fisik" 
              colorClass="text-navy"
            />
            <p className="text-sm text-gray-500 mt-2 text-center">
              {data.paket_selesai} dari {data.total_paket} rumah selesai dikerjakan
            </p>
          </div>
          <div className="flex flex-col items-center">
            <ProgressRadial 
            ...
            />
            <p className="text-sm text-gray-500 mt-2 text-center">
              Telah dibayarkan: {formatRupiah(data.total_nilai_terbayar)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        />
        <StatCard 
          title="Selesai 100% (Fisik)" 
          value={data.paket_selesai} 
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-display font-bold text-navy">Perhatian: Sisa Tagihan Terbesar</h2>
          </div>
        </div>
        ...
        {topUnpaid.length === 0 ? (
          ...
          <div className="divide-y divide-gray-50">
            {topUnpaid.map((paket) => {
              ...
              return (
                <div key={paket.id} className="p-6 hover:bg-gray-50/50 transition-colors flex items-center justify-between group">
                  <div>
                    <h3 className="font-semibold text-navy mb-1 group-hover:text-gold transition-colors">{paket.nama_paket}</h3>
                    <p className="text-sm text-gray-500">{paket.lokasi || 'Lokasi belum diset'}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-mono text-sm text-gray-500 mb-1">
                      Sisa: <span className="font-bold text-red-600">{formatRupiah(sisaTagihan)}</span>
                    </p>
